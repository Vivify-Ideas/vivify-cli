  ...
  if (data instanceof Array) {
    return data.map(item => {
      if (item instanceof Array) {
        return item.map(stringElement => chalk.hex(color)(stringElement))
      }
...
function drawBoardTable(boardData, numberOfColumns) {
  let columns = boardData.columns.slice(
    boardData.columns.length < numberOfColumns ? 0 : boardData.columns.length - numberOfColumns,
    boardData.columns.length
  )
  let tableInstance = new table({
    head: columns.map(column =>
      chalk.hex(column.status === 'active' ? ITEM_TYPE_COLORS.Story : ITEM_TYPE_COLORS.Bug)(
  ...
  let columnItems = columns.map(col =>
    col.items.map(item => colorByItemType(item.type, wrap(`${item.code} ${item.name}`.trim(), 10)))
  )
  let items = []
  let maxItems = columnItems.map(items => items.length).reduce((val1, val2) => Math.max(val1, val2))
...
function drawBoardItem(item) {
  ...
  let coloredItemData = colorByItemType(item.type, [
    [`Code: ${item.code}`],
  ...
  tableInstance.push(...coloredItemData)
  console.log(tableInstance.toString())
}
